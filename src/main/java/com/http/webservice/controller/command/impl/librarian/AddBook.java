        ...
        try {
            if (user.getAccessLevel() > 1) {
                service.addToMainLib(request.getParameter("title"), request.getParameter("author"),
                        Integer.parseInt(request.getParameter("issue")), Float.parseFloat(request.getParameter("cost")),
                        Integer.parseInt(request.getParameter("rating")), Float.parseFloat(request.getParameter("rent_cost")));
